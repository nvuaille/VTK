...
//----------------------------------------------------------------------------
template<typename T> const vtkQuaternion<T>& vtkDualQuaternion<T>::GetReal() const
{
  return this->Real;
}

//----------------------------------------------------------------------------
template<typename T> const vtkQuaternion<T>& vtkDualQuaternion<T>::GetDual() const
{
  return this->Dual;
}
...
//----------------------------------------------------------------------------
template<typename T> vtkDualQuaternion<T> vtkDualQuaternion<T>::Normalized() const
{
  vtkDualQuaternion<T> q(*this);
  q.Normalize();
  return q;
}
...
::SetRotationTranslation(const vtkQuaternion<T>& rotation, const T translation[3])
{
  /*
  vtkQuaternion<T> translation(
    ...
    */
  vtkQuaternion<T> dual(0.f, translation);
  *this = vtkDualQuaternion<T>(rotation, (dual * rotation) / 2.f) ;
}
...
::SetRotation(const vtkQuaternion<T>& rotation)
{
  ...
  this->SetRotationTranslation(rotation, origin);
}
...
{
  this->SetRotationTranslation(vtkQuaternion<T>(), translation);
}
...
::ScLerp(const T& t, const vtkDualQuaternion<T>& _dq)const
{
  ...
  // Shortest path
  T dot = this->Real.Dot(_dq.Real);
  vtkDualQuaternion dq = dot >= 0. ? _dq : _dq * -1.;

  vtkDualQuaternion<T> diff = this->Conjugated() * dq;
  T vr[3];
  vr[0] = diff.GetReal().GetX();
  vr[1] = diff.GetReal().GetY();
  vr[2] = diff.GetReal().GetZ();

  T vd[3];
  vd[0] = diff.GetDual().GetX();
  vd[1] = diff.GetDual().GetY();
  vd[2] = diff.GetDual().GetZ();

  T invr = 1. / sqrt(vtkMath::Dot(vr, vr));

  T angle = 2. * acos(diff.GetReal().GetW());
  T pitch = -2. * diff.GetDual().GetW() * invr;
  T direction[3];
  direction[0] = vr[0] * invr;
  direction[1] = vr[1] * invr;
  direction[2] = vr[2] * invr;
  T moment[3];
  moment[0] = (vd[0] - direction[0] * pitch * diff.GetReal().GetW()*0.5)*invr;
  moment[1] = (vd[1] - direction[1] * pitch * diff.GetReal().GetW()*0.5)*invr;
  moment[2] = (vd[2] - direction[2] * pitch * diff.GetReal().GetW()*0.5)*invr;

  angle *= t;
  pitch *= t;
  T sinAngle = sin(0.5*angle);
  T cosAngle = cos(0.5*angle);
  
  vtkQuaternion<T> real (cosAngle, direction[0] * sinAngle, direction[1] * sinAngle, direction[2] * sinAngle);
  vtkQuaternion<T> dual (-pitch*0.5*sinAngle,
                        sinAngle*moment[0]+pitch*0.5*cosAngle*direction[0],
                        sinAngle*moment[1]+pitch*0.5*cosAngle*direction[1],
                        sinAngle*moment[2]+pitch*0.5*cosAngle*direction[2]);
  return *this * vtkDualQuaternion<T>(real, dual);
}
...
//----------------------------------------------------------------------------
template<typename T> vtkDualQuaternion<T> vtkDualQuaternion<T>
::ScLerp3(const T& t, const vtkDualQuaternion<T>& _dq)const
{
  if (t == 0.)
    {
    return *this;
...
//----------------------------------------------------------------------------
template<typename T> void vtkDualQuaternion<T>::ToMatrix4x4(vtkMatrix4x4* output) const
{
  vtkDualQuaternion<T> q = this->Normalized();
  T w = q.GetReal().GetW();
  T x = q.GetReal().GetX();
  T y = q.GetReal().GetY();
  T z = q.GetReal().GetZ();
 ...
 // Extract rotational information
  output->SetElement(0, 0, w*w + x*x - y*y - z*z);
  output->SetElement(1, 0, 2*x*y + 2*w*z);
  output->SetElement(2, 0, 2*x*z - 2*w*y);

  output->SetElement(0, 1, 2*x*y - 2*w*z);
  output->SetElement(1, 1, w*w + y*y - x*x - z*z);
  output->SetElement(2, 1, 2*y*z + 2*w*x);

  output->SetElement(0, 2, 2*x*z + 2*w*y);
  output->SetElement(1, 2, 2*y*z - 2*w*x);
  output->SetElement(2, 2, w*w + z*z - x*x - y*y);

  // Extract translation information
  T translation[3];
  q.GetTranslation(translation);
  output->SetElement(0, 3, translation[0]);
  output->SetElement(1, 3, translation[1]);
  output->SetElement(2, 3, translation[2]);
}


//----------------------------------------------------------------------------
template<typename T> void vtkDualQuaternion<T>::FromMatrix4x4(const vtkMatrix4x4* input)
{
  vtkQuaternion<T> rotation;
  rotation.FromMatrix4x4(input);

  double translation[3];
  translation[0] = input->GetElement(0, 3);
  translation[1] = input->GetElement(1, 3);
  translation[2] = input->GetElement(2, 3);

  SetRotationTranslation(rotation, translation);
}
